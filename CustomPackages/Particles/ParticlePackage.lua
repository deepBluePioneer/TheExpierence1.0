...
function ParticleClass:Update(delta)
	...
	local transparencyValue = evalNS(self.Transparency, Normalize(0, self.maxAge, self.Age))
	if type(transparencyValue) == "number" then
		self.Instance.Transparency = transparencyValue
	else
		-- Handle the case where no valid transparency value is returned
		self.Instance.Transparency = 1 -- Default to fully transparent or another suitable default
	end
