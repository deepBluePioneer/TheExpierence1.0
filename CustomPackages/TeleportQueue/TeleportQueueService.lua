...
function TeleportQueue:Flush(): (FlushResult, (TeleportAsyncResult | string)?)
	...
	local wasSuccessful, teleportResult = pcall(function()
		...
		local code = TeleportService:ReserveServer(queuePlaceId)
		
		return TeleportService:TeleportToPrivateServer(queuePlaceId, code, actualPlayersTeleporting,teleportOptions)
		--return TeleportService:TeleportAsync(queuePlaceId, actualPlayersTeleporting, teleportOptions)
	end)
