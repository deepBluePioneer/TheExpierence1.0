...
function GraviBowToolController:KnitStart()
	...
	self._playerService.HarvesterTarget:Connect(function(harvesterObj, targetPos)
		self:_onHarvesterTarget(harvesterObj, targetPos)
	end)

	self._playerService.HarvesterMining:Connect(function(harvesterObj, isMining)
		...
		if dist <= CRYSTAL_MINE_RANGE then
			...
			if crystalModel then
				if self._miningTarget ~= crystalModel then
					...
					self._scannerBeamSound = self._soundController:PlayNamedLoopGlobal("HarvestBeam", "scanner")
				else
...
function GraviBowToolController:_destroyCrystal(crystalModel)
	...
	self._soundController:PlayGlobal("HarvestCollected")
...
function GraviBowToolController:_onHarvesterTarget(harvesterObj, targetPos)
...
function GraviBowToolController:_cleanupHarvester(harvesterObj)
	local entry = self._activeHarvesters[harvesterObj]
...
function GraviBowToolController:_onHarvesterMining(harvesterObj, isMining)
	local entry = self._activeHarvesters[harvesterObj]
	...
	if isMining then
		...
		if rootPart then
			...
			if entry then
				...
				self._harvesterBeamSounds = self._harvesterBeamSounds or {}
		...
		if entry and entry.beamSound then
			...
			entry.beamSound:Destroy()
			entry.beamSound = nil
		elseif self._harvesterBeamSounds and self._harvesterBeamSounds[harvesterObj] then
...
function GraviBowToolController:_setupHarvesterPhysicsLoop()
