...
function GraviBowPlayerService:KillAndRespawn(player)
...
function GraviBowPlayerService:_scheduleRespawn(player)
...
function GraviBowPlayerService:_setupCharacter(player, character)
	...
	if not character.Parent or character ~= player.Character then return end
...
function GraviBowPlayerService:_dropPlayerOnPlanet(player, planetData)
	...
	local spawnPos
	if result then
		spawnPos = result.Position + upDir * 5
	else
		spawnPos = center + upDir * (radius + 10)
	end
...
function GraviBowPlayerService:TeleportPlayerToPlanet(player, planetData)
	self:_dropPlayerOnPlanet(player, planetData)
end
