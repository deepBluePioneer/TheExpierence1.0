...
function CTF_TeleportService:InitReplicas()
...
function CTF_TeleportService:KnitInit()
    self:InitReplicas()
...
function CTF_TeleportService:HandlePlayerEntered(teleporter, player)
...
function CTF_TeleportService:HandleQueueResult(teleporter, player, result)
    local teleportData = self.teleporters[teleporter]
...
function CTF_TeleportService:handleUnsuccessfulAdd(result, player, teleporterName)
    if result == "TeleportQueue is full" then
        warn(teleporterName .. " queue is full. Player " .. player.Name .. " could not be added.")
    end
end

function CTF_TeleportService:HandlePlayerExited(teleporter, player)
    local teleportData = self.teleporters[teleporter]
...
function CTF_TeleportService:startTimer(teleporter)
    local teleportData = self.teleporters[teleporter]
    if teleportData.gameInSession then
...
function CTF_TeleportService:setupNewTimer(teleportData, teleporter)
...
function CTF_TeleportService:timerTick(teleportData, teleporter)
    if teleportData.zoneCounter > 0 then
...
function CTF_TeleportService:startGameTimer(teleportData, teleporter)
...
function CTF_TeleportService:setupNewGameTimer(teleportData, teleporter)
...
function CTF_TeleportService:timerGameTick(teleportData, teleporter)
...
function CTF_TeleportService:startReturnTimer(teleportData, teleporter)
...
function CTF_TeleportService:setupNewReturnTimer(teleportData, teleporter)
...
function CTF_TeleportService:timerReturnTick(teleportData, teleporter)
...
function CTF_TeleportService:returnPlayersToOriginalLocations(teleporter)
    local teleportData = self.teleporters[teleporter]
...
function CTF_TeleportService:processWaitingPlayers(teleporter)
    local teleportData = self.teleporters[teleporter]
...
function CTF_TeleportService:TeleportToPlace(teleportQueue, teleportData)
...
function CTF_TeleportService:executeTeleport(teleportData, teleporter)
    if not RunService:IsStudio() then
        warn("Teleporting...")
        self:TeleportToPlace(teleportData, teleporter)
...
function CTF_TeleportService:Teleport(teleportQueue, spawnLocations, teleportData)
...
function CTF_TeleportService:KnitStart()
    -- Expose the TimeUpSignal for other services
    function CTF_TeleportService:GetTimeUpSignal()
        return self.OnLobbyTimerEnd
    end

    function CTF_TeleportService:GetOnGameTimerEnd()
        return self.OnGameTimerEnd
    end

    function CTF_TeleportService:GetOnReturnTimerEnd()
        return self.OnReturnTimerEnd
    end
end

return CTF_TeleportService
